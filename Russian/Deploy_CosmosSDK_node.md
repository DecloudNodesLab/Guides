...
При разверке доступны следующие переменные в `deploy.yml`: <br/>
```
- "SSH_PASS="  - Пароль для подключения по SSH (пользователь `root`) <br/>
- "MONIKER=" - имя ноды.<br/>
- "VALIDATOR_KEY_JSON_BASE64=" - Вставьте содержимое файла `priv_validator_key.json` зашифрованное с помощью **BASE64** *.<br/>

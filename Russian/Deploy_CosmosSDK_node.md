...
- "BINARY_VERSION=" Версия или коммит бинарного файла, который вы хотите скомпилировать(может использываться только с заполненным GITHUB_REPOSITORY).
...
- "PRUNING=off" - отключение пруннинга. Если оставить переменную пустой - пруннинг будет включен, можно добавить занчения  KEEP_RECENT и INTERVAL при необходимости.
- "SNAPSHOT_INTERVAL" - в ручную задать интервал снепшотов, по-умолчанию значение равно 2000.

* При использовании BINARY_LINK необзодимо дополнительно добавить переменную BINARY - содержащую имя бинарного файла. Если ее не установить - имя бинарного файла будет задано как "serviced".
